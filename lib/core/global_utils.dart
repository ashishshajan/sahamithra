...
class GlobalUtils {
  ...
  static const String _keyGuestDob = 'guest_dob';
  ...
  bool get isGuestUser => _prefs.getBool(_keyIsGuestUser) ?? false;

  /// Public explore flow without a bearer token: use `/guest-scales` and `/guest-scale-scores`.
  /// When a session token exists (logged-in parent), use authenticated scale APIs instead.
  bool get isGuestScaleSession => isGuestUser && (token?.isEmpty ?? true);

  /// Guest session: child DOB as `dd-MM-yyyy` for guest-scales API.
  String? get guestDob => _prefs.getString(_keyGuestDob);
  ...
  Future<void> setGuestUser(bool value) async {
    await _prefs.setBool(_keyIsGuestUser, value);
    if (!value) {
      await _prefs.remove(_keyGuestDob);
    }
  }

  Future<void> setGuestDob(String value) async =>
      await _prefs.setString(_keyGuestDob, value);
