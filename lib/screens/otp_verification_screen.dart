...
class _OTPVerificationScreenState extends State<OTPVerificationScreen> {
  ...
  final List<GlobalKey> _otpFieldKeys = List.generate(6, (_) => GlobalKey());
  final GlobalKey _verifyButtonKey = GlobalKey();
  bool _otpViewportAdjusted = false;
  ...
  void _setupFocusListeners() {
    for (int i = 0; i < 6; i++) {
      final index = i;
      final listener = () => _handleOtpFocusChange(index);
      _focusListeners.add(listener);
      _focusNodes[i].addListener(listener);
    }
  }

  void _handleOtpFocusChange(int index) {
    if (_focusNodes[index].hasFocus && index == 0 && !_otpViewportAdjusted) {
      _otpViewportAdjusted = true;
      Future.delayed(
        const Duration(milliseconds: 250),
        () => _ensureOtpFieldAndButtonVisible(index),
      );
    }
  }

  Future<void> _ensureOtpFieldAndButtonVisible(int index) async {
    final fieldContext = _otpFieldKeys[index].currentContext;
    ...
    final buttonContext = _verifyButtonKey.currentContext;
  ...
  Future<void> _handleVerify() async {
    ...
    if (result['success']) {
      ...
      _showSnack(
        isExistingUser
            ? lang.t('otpWelcomeBackLoggingIn')
            : (data?['message']?.toString() ??
                lang.t('otpVerifiedSettingUpAccount')),
        isError: false,
      );
      ...
      setState(() => _isLoading = false);
      _showSnack(
        result['message']?.toString() ?? lang.t('otpVerificationFailed'),
        isError: true,
      );
    }
  }
  ...
  Future<void> _handleResend() async {
    ...
    if (result['success']) {
      ...
      _showSnack(lang.t('otpResentSuccessToast'), isError: false);
    } else {
      _showSnack(
        result['message']?.toString() ?? lang.t('otpResendFailed'),
        isError: true,
      );
    }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 2.h),
                    Text(
                      lang.t('otpEnterCodePrompt'),
                      maxLines: 3,
                      overflow: TextOverflow.ellipsis,
                      softWrap: true,
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        fontSize: lang.isMalayalam ? 10.5.sp : 11.sp,
            ...
            child: _canResend
                ...
                : RichText(
                    text: TextSpan(
                      ...
                      children: [
                        TextSpan(text: lang.t('otpResendInPrefix')),
                        TextSpan(
                          text: '0:$_resendTimer',
                        ...
                        TextSpan(text: lang.t('otpResendInSuffix')),
                      ],
                    ),
                  ),
          ),
