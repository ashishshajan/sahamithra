...
class _TherapyVideosScreenState extends State<TherapyVideosScreen>
  ...
  /// Holds a translation key (preferred) or a raw error message returned by the
  /// backend. The UI resolves translation keys at render time so that toggling
  /// the app language updates any error state in place.
  String? _errorKey;
  String? _errorRawMessage;
  ...
  Future<void> _loadLibraries() async {
    setState(() {
      _isLoading = true;
      _errorKey = null;
      _errorRawMessage = null;
    });

    final utils = GlobalUtils();
    final Map<String, dynamic> result;
    if (utils.isGuestUser) {
      result = await NetworkHelper().getGuestLibraries();
    } else {
      final childId = utils.childId;
      if (childId == null) {
        setState(() {
          _isLoading = false;
          _errorKey = 'teamErrorChildIdNotFound';
        });
        return;
      }
      result = await NetworkHelper().getPatientLibraries(
        childId: childId,
        viewMode: 'patient',
      );
    }
    ...
    if (result['success'] != true) {
      ...
      setState(() {
        ...
        if (msg != null && msg.isNotEmpty) {
          _errorRawMessage = msg;
        } else {
          _errorKey = 'therapyLibrariesLoadFailed';
        }
      });
      return;
    }
    ...
    if (data is! List) {
      setState(() {
        _isLoading = false;
        _errorKey = 'therapyVideosNotFound';
      });
      return;
    }
    ...
    if (newTabs.isEmpty) {
      setState(() {
        ...
        _modules = {};
        _isLoading = false;
        _errorKey = 'therapyLibrariesNotFound';
      });
      return;
    }
  ...
  String _resolvedErrorMessage(LanguageProvider lang) {
    if (_errorKey != null) return lang.t(_errorKey!);
    return _errorRawMessage ?? '';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        // Touch the language Rx so this screen rebuilds when language changes.
        final _ = lang.language;
        final hasError = _errorKey != null || _errorRawMessage != null;
        return Column(
          children: [
            GradientHeader(
              onBack: () => Get.back(),
              title: lang.t('homeTherapyVideos'),
              subtitle: lang.t('therapyVideosScreenSubtitle'),
            ),
            ...
            if (!_isLoading && _tabs.isNotEmpty)
              ...
              child: _isLoading
                  ...
                  : hasError
                      ...
                      : (_tabs.isEmpty || _tabController == null)
                          ? Center(
                              child: Text(
                                lang.t('therapyLibrariesNotFound'),
                                style: TextStyle(
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Obx(
                    () => Text(
                      LanguageProvider.to.t('therapyWatchModule'),
                      style: TextStyle(
                        fontSize: 13.sp,
                        fontWeight: FontWeight.w600,
                        color: Colors.white,
