...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Future<void> _showChooseChildSheet() async {
    ...
    if (children.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(lang.t('accountNoChildren'))),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final childName = utils.childName ?? '';
    final parentLabel = utils.parentName ?? '';
    final phone = utils.phoneNumber ?? '';
    final address = (userMap?['address'] ?? '').toString();
    final gender = utils.childGender ?? '';
    final dob = (utils.childDob != null && utils.childDob!.trim().isNotEmpty)
        ? _formatDobForDisplay(utils.childDob!)
        : '';
                  ...
                  children: [
                    ...
                    _AccountMenuTile(
                      ...
                      onTap: () async {
                        ...
                        if (confirmed && mounted) await _logout();
                      },
                    ),
