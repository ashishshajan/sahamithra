          ...
          webController
            ...
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                ...
                final num rate = raw is num
                    ? raw
                    : num.tryParse(raw.toString()) ?? 1.0;
                controller!.updateValue(
            ...
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                // iOS WebView may send null for position/buffered during init or edge cases.
                if (args.isEmpty) return;
                final dynamic first = args.first;
                final dynamic last =
                    args.length > 1 ? args.last : null;
                if (first == null) return;
                final num? seconds = first is num
                    ? first
                    : num.tryParse(first.toString());
                if (seconds == null) return;
                final position = seconds * 1000;
                final num buffered = last == null
                    ? 0.0
                    : (last is num
                        ? last
                        : num.tryParse(last.toString()) ?? 0.0);
                controller!.updateValue(
                  controller!.value.copyWith(
